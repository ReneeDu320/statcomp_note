...
helper <- function(y, theta, gammai, log=F){
...
inner <- function(theta, node, y, pm, sd_hess, log=F){
  ...
  for(i in seq(length(node))){
    val[i] <- helper(y, theta, gammai = node[i])/dnorm(node[i], pm, sd_hess)
  }
  if(log==F){
    return(val)
  } else{
...
log1<- function(theta, y,s=s, w=w){
 wt <- w/sqrt(pi)
 pm <- optimx(par = 0, fn = function(x, y, theta) {-helper(gammai = x,y = y,theta = theta)}, y = y, theta = theta,
      method = "Nelder-Mead")
 pm <- pm$p1
 hess_pm <-  hessian(func = helper, x = pm, y = y, theta = theta, log = T)
...
log2 <- function(theta, data, M){ 
  gh <-  gauss.quad(n = M, kind = "hermite")
  s <-  gh$nodes
  w <-  gh$weights
  val <- 0
  for (i in 1:max(alz$subject)){
    val = val + log1(y=data$words[data$subject == i], theta = theta, s=s, w=w)
  }
  return(val)
}
## End Solution
...
for (M in 1:10){
  val <- log2(theta = c(beta, s2gamma),data=alz,M=M)
  print(paste0( M ,val)) 
}
```
Based on the result, I will choose M=5
```{r,,warning=FALSE}
theta = c(beta, s2gamma)
#result <- optimx(par=theta, f = function(x, data, M)
#  {-log2(theta=x, data, M)}, method="Nelder-Mead",data = alz, M=5)
## End Solution
